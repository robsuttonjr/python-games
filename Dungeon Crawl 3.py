...
======================================================
**Mega update** (based on your “all you got” ask):
- **Minimap** (top-right): shows explored tiles, player, enemies, stairs.
- **Dash** (Left Shift): short burst + brief invulnerability (i-frames) with cooldown.
- **Boss on Level 3**: big creature with a ranged attack pattern. Beat it to loop back stronger.
- **HP Colors**: green→yellow→red as enemies weaken.
- **Pickups**: Damage Boost (orange) & Shield (blue).
- **Wider halls**, scenery (pillars/crates), **3 dungeon levels** with stairs.
- **Sprites**: simple person-like player & creature variants (eyes/horns/mandibles).
- **QoL**: Aim reticle, pause menu (P), help overlay (F1), difficulty selector (1=Easy,2=Normal,3=Hard) at start.
...
Notes:
- Single file with primitive art; easy to swap sprites later.
"""
import math
import random
import sys
...
DIFFICULTY = {
    "Easy":    {"enemy_hp":0.9, "enemy_dmg":0.8, "enemy_speed":0.95, "max_enemies":5},
    "Normal":  {"enemy_hp":1.0, "enemy_dmg":1.0, "enemy_speed":1.0,  "max_enemies":6},
    "Hard":    {"enemy_hp":1.2, "enemy_dmg":1.2, "enemy_speed":1.05, "max_enemies":8},
}
...
SPAWN_INTERVAL = 6.0
WAVE_SCALE = 1.10
MAX_ACTIVE_ENEMIES = 6  # overridden by difficulty
...
class Game:
    ...
    def _draw_minimap(self):
        ...
        for e in self.enemies:
            if not e.alive: continue
            pygame.draw.circle(surf, (220,120,120), (int(e.pos.x/(TILE)*sx), int(e.pos.y/(TILE)*sy)), 2)
        # stairs
        if self.dungeon.stairs_tx is not None:
